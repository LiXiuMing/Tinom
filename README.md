...
## 待办事项
- __[Bug]__ 作好事未遂:反馈走动有时会出现卡顿.
    - 解决:加入名单数量显示与自定义限制最大数量,
    - 监狱功能:屏蔽一段时间后将其释放出来,以减轻名单压力,再犯加长监禁时间.
- [Func]右键加入黑名单
- [Func]关闭系统关键词过滤:C_CVar.SetCVar("profanityFilter",1);value = C_CVar.GetCVar("name")
- [Func]系统自动角色名染色:C_CVar.SetCVar("colorChatNamesByClass",1)
- [Func]重复发言自定义时间间隔
...
3. 聊天增强:(吃瓜模式,强力吃瓜模式,卖瓜模式)
    ...
    - 笔记功能
    - 聊天窗口姓名前加上职业或者职责(UI escape sequences-AtlasID)
...
7. 清空上次密语玩家功能,有人不想再敲空格还是回复上次的密语人,想改成上次的频道
